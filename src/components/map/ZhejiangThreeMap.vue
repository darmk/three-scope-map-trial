...
function updateCityLabelPresentation() {
  ...
  const horizontalPadding = 5 * labelScale;
  ...
  style.setProperty('--map-label-padding-x', `${horizontalPadding.toFixed(1)}px`);
  ...
  cityLabelElements.forEach((element, labelName) => {
    const characterCount = Array.from(labelName.trim()).length;
    const readableWidth = characterCount * baseFontSize
      + horizontalPadding * 2
      + 12 * labelScale;
    const itemWidth = Math.min(baseWidth * 2.1, Math.max(baseWidth, readableWidth));
    element.style.setProperty('--map-label-item-width', `${itemWidth.toFixed(1)}px`);
    element.style.setProperty('--map-label-item-selected-width', `${(itemWidth * selectedScale).toFixed(1)}px`);
  });
}
...
function getScopeTransform() {
  ...
  return {
    ...
    scale: currentState.scope === 'country' ? 0.84 : 0.768,
  };
}
...
async function createMap() {
  ...
  const topMaterial = createMapTerrainMaterial({
    elevationScale: currentState.scope === 'country' ? 6.8 : currentState.scope === 'province' ? 8.2 : 7.2,
    normalStrength: currentState.scope === 'country' ? 0.96 : 1,
    roughness: currentState.scope === 'country' ? 0.88 : 0.94,
    textureOpacity: currentState.scope === 'country' ? 0.88 : 0.9,
  });
...
.map-host :deep(.city-label) {
  ...
  width: var(--map-label-item-width, var(--map-label-width, 51px));
  ...
  text-shadow:
    ...
    0 0 5px var(--map-label-text-glow),
    0 0 10px var(--map-label-text-glow);
  opacity: 0.98;
...
.map-host :deep(.city-label.is-selected) {
  width: var(--map-label-item-selected-width, var(--map-label-selected-width, 60.2px));
